...
#------------------------------------------------------------------#
# TITLE:        Week6exercise_crosscorrelation_function.R
#
...
# DATE:         8 Oct 2014
#
# COMMENTS:     This file contains Assignment #4's
#               cross-correlation function, to be used to cross 
#               correlate a multi-variate file.
#
#
#-----Setting the working directory and clearing the workspace-----#

CrossCorrelationFunction <- function(thedata) {

vars = thedata[1,1:ncol(thedata)] 
datain <- names(thedata)
  ...
  for (i in 1:n){ # For each variable
    if (i <= j){ # If i is less than j...
      ...
      if (j == i){       # If i and j are the same, then make a 
        ...
        text(min(x)+2*(range(x,na.rm=T)[2] - range(x,na.rm=T)[1])/10, 
             # place text in each figure.
             max(y)-2*(range(y,na.rm=T)[2] - range(y,na.rm=T)[1])/10,   
...
# Here, the negative index (e.g. gridLoc[-index,1])  
# indicates that those are indices you do not want to inlcude. 
...
#for (i in 1:length(index)){  # For each grid location in the index  
  # used, add text to the plot that shows the grid number. 
