...
# DATE:         23 Sept. 2014
#
# COMMENTS:     This file contains code to read in one dataset - random seedling data.  The intent of this script is 
#               to examine the dataset and determine if:
...
#dimnames(density_spp) <- list(NSites[1:30],NClasses[1:3],NSPECIES[1:6])

#----------Plotting----------------------------------------------------------------------------------------------------#

#-----------Plotting for Summary Seedling Distribution by Class and Species--------------------------------------------#
...
#--For Loops that create boxplots showing distribution of seedling density & mean seeding height by class and species--# 

win.graph(11,10) #---Creates the window size---------------------------------------------------------------------------#
bins = seq(0,6000,500) #-----defining the bin sequences----------------------------------------------------------------#
par(mfrow=c(1,6))  #---par function sets number or rows and columns for plotting---------------------------------------#

#---creates a list of species names------------------------------------------------------------------------------------#

NSPECIES <- list (a = "Pico", b = "Pipo", c = "Pien", d = "Psme", e = "Alba", f = "All Species")
specieslist <- c(NSPECIES$a, NSPECIES$b, NSPECIES$c, NSPECIES$d, NSPECIES$e, NSPECIES$f)

#--------------For Loop1 that creates boxplots for the density per site, for each species, stratified by class---------#

    for (k in NSpp) {
        boxplot(density_spp[1:30, 1:3, k],
        at = 1:3 + k,
        ylim = c(0,.7),
        col="aquamarine3",
        xlab="Class",
...
#-------For Loop2 that creates boxplots for the mean seedling ht per site, for each species, stratified by class ------#

    for (k in NSpp) {
      boxplot(mean_Ht[1:30, 1:3, k],
              at = 1:3 + k,
              ylim = c(0,60),
              col="lightgoldenrod",
              xlab="Class",
