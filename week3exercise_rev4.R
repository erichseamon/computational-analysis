...
#dirname <- setwd("M:/obj1/netcdf/MET")   #----set the working directory
dirname <- setwd("X:/Dropbox/ES Research/ES Classwork/FOR504/Data")   #----set the working directory
...
for (i in yearspan) {
  for (j in variablespan) {
    setwd(paste("M:/obj1/netcdf/MET/", j, sep = ""))
    dirname <- "M:/obj1/netcdf/MET"
    aggmetnc <- file.path(paste(j, "_", i, ".nc", sep = ""))
