...
mu = mean(var1)  # Set the mean.
sigma = sd(var1) # Set the standard deviation.
randNorm = rnorm(n, mu, sigma) # Use rnorm function to generate random numbers.


muExp = mean(var1)  # Set the mean.
...
fit = fitdistr(var1, "Poisson")
lambdaHat = fit$estimate
nHat = dpois(var1,lambdaHat)

win.graph()
hist(var1, breaks = c(min(var1):max(var1)), freq = FALSE, col=rgb(0.75,0.75,0.75),
     ylab = "Probability density",
     xlab = "x",
     main = paste("lambda = ", lambda))
points(var1,nHat,lwd=2)
text(min(var1),max(nHat), pos = 4,
